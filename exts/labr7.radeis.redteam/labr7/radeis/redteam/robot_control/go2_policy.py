...
class GO2FlatTerrainPolicy(PolicyController):
    ...
    def __init__(
        ...
        Args:
            ...
            policy_path (str) -- path to the policy.pt file
            env_config_path (str) -- path to the env.yaml file
            position (np.ndarray) -- position of the robot
            orientation (np.ndarray) -- orientation of the robot

        """
        if usd_path is None:
            usd_path = "/isaacsim/GO2/go2.usd"

        if policy_path is None:
            policy_path = "/isaacsim/GO2/policy/policy.pt"

        if env_config_path is None:
            env_config_path = "/isaacsim/GO2/policy/env.yaml"

        if "name" in inspect.signature(PolicyController.__init__).parameters:
