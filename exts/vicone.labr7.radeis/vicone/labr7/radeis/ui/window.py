...
class RadeisRedTeamWindow:
    def __init__(self, ext_path: str):
        ...
        self._contact_icon = os.path.join(self._data_dir, "icon", "email_icon.svg")
    ...
    def _read_settings(self):
        try:
            ...
            v = s.get("/exts/vicone.labr7.radeis/sidecar_url")
                ...
                with ui.ScrollingFrame() as self._main_scroll_frame:
                    with ui.ZStack():
                        ...
                        self._grayout_overlay = ui.Rectangle(
                            style={"background_color": 0x00000000})
                        # Host layer for the onboarding intro card. It MUST
                        # live inside the ScrollingFrame: a ZStack sibling
                        # placed after a ScrollingFrame is never drawn over
                        # it - verified live on 2026-08-31, neither a
                        # Rectangle nor a Frame painted from the root ZStack.
                        # Here it is a sibling of the content VStack, the
                        # exact position that makes _grayout_overlay work.
                        # It is CANVAS-sized, not viewport-sized; hiding the
                        # content while the card is up collapses the canvas to
                        # the viewport, and _open_onboarding rewinds scroll_y
                        # first so the card cannot open off-screen.
                        self._intro_overlay = ui.Frame(visible=False)
    ...
    def _banner(self):
        with ui.ZStack(height=R.HEIGHT_BANNER):
            ...
            with ui.HStack(spacing=R.SPACING_BANNER, style=R.STYLE_BANNER_HSTACK):
                ...
                with ui.VStack(width=0, height=R.HEIGHT_BANNER, spacing=0,
                               style={"margin": 0}):
                    if R.PADDING_INTRO_BTN_STACK_TOP:
                        ui.Spacer(height=R.PADDING_INTRO_BTN_STACK_TOP)
                    self._intro_btn = R.intro_button(
                        ...
                        center_vertically=False, pill_width=R.WIDTH_INTRO_BTN_STACK)
                    ui.Spacer(height=R.SPACING_INTRO_BTN_STACK)
                    self._contact_intro_btn = R.intro_button(
                        "Contact", self._on_open_get_in_touch,
                        icon_path=self._contact_icon if os.path.exists(self._contact_icon) else "",
                        tooltip="Contact the Radeis team", tooltip_width=190,
                        center_vertically=False, pill_width=R.WIDTH_INTRO_BTN_STACK)
                    if R.PADDING_INTRO_BTN_STACK_BOTTOM:
                        ui.Spacer(height=R.PADDING_INTRO_BTN_STACK_BOTTOM)
    ...
    # -------------------------------------------------------- get in touch
    def _on_open_get_in_touch(self):
        """"Contact" pill - open (or re-focus) the Get in Touch form.

        Deliberately NOT show_contact_us(): that is the read-only coming-soon
        pitch used by seven upsell call sites and stays as-is. Owned as an
        instance attribute (not a module singleton) so destroy() can reach it
        on extension reload. Non-modal, and the main panel is NOT dimmed -
        this is a side form, not a takeover flow like onboarding.
        """
        try:
            if self._get_in_touch is None:
                from .get_in_touch_win import GetInTouchWin
                self._get_in_touch = GetInTouchWin()
            self._get_in_touch.show()
        except Exception:  # noqa: BLE001
            # NEVER drop the reference without tearing the window down: a live
            # title-keyed ui.Window with no Python reference is the documented
            # critical orphan leak (orphan-modelwizard-window-leak.md) - Kit
            # keeps rendering it, destroy() can no longer reach it, and the
            # next click builds a second window fighting it for the same
            # title-keyed geometry.
            win = self._get_in_touch
            self._get_in_touch = None
            if win is not None:
                try:
                    win.destroy()
    ...
    def _open_onboarding(self):
        if self._onboarding is not None:
            return
        # Onboarding is a takeover flow (dim overlay + disabled main content);
        # Get in Touch is a side form. Leaving the side form open means two
        # windows competing for attention, with the form stacked over the card
        # it is meant to yield to. hide() is not destroy(): the unsent draft
        # survives and the Contact pill reopens it unchanged.
        if self._get_in_touch is not None:
            try:
                self._get_in_touch.hide()
            except Exception:  # noqa: BLE001
                pass
        # The overlay is sized to the scroll CANVAS, not the viewport, so a
        # card built while the panel is scrolled down would sit above the
        # visible area. Rewind first - the panel is about to be disabled
        # anyway, so nothing is lost.
        try:
            self._main_scroll_frame.scroll_y = 0
        except Exception:  # noqa: BLE001
            pass
        from ..onboarding.window import OnboardingWindow
        self._onboarding = OnboardingWindow(
            ext_path=self._ext_path, host=self._intro_overlay,
            on_finish_fn=self._on_onboarding_finished)
        self._onboarding.show()
        # visible=False, not just enabled=False. A ScrollingFrame's content is
        # drawn in a LATER pass than a ZStack layer stacked above it, so the
        # panel's two nested ScrollingFrames (the sign grid and the log) punch
        # straight through the card - verified live on 2026-08-31, the action
        # chips rendered on top of an opaque card background whose measured
        # rect fully covered them. Hiding the content is what stops that, and
        # it also collapses the scroll canvas to the viewport, so the
        # canvas-sized overlay can no longer scroll out from under the card.
        # enabled=False and the dim are kept as belt-and-braces for the frame
        # where visibility has not applied yet.
        try:
            self._main_content_vstack.visible = False
            self._main_content_vstack.enabled = False
            self._grayout_overlay.style = {"background_color": R.COLOR_GRAYOUT_OVERLAY}
        except Exception:  # noqa: BLE001
            pass
    ...
    def _on_onboarding_finished(self):
        # One guard per restore, visibility first. A panel left invisible has
        # no UI left to recover it, so that restore must never be skippable by
        # an earlier line raising - which is exactly what a single shared
        # try/except would allow.
        try:
            self._main_content_vstack.visible = True
        except Exception:  # noqa: BLE001
            pass
        try:
            self._main_content_vstack.enabled = True
