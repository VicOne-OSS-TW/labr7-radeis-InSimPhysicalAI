"""IExt lifecycle entry point for vicone.labr7.radeis (Radeis Red-Team v0.2).
...
# Mirrors onboarding/window.py::WIN_TITLE. Kept as a literal so the
# sweep never depends on importing the onboarding module.
_MENU_ONBOARDING = "Radeis - Introduction"
# Mirrors ui/get_in_touch_win.py::WIN_TITLE. Kept as a literal so the
# sweep never depends on importing that module.
_MENU_GET_IN_TOUCH = "Get in Touch"
...
class LabR7IssacExtension(_EXT_BASE):
    ...
    def _sweep_stale_windows(self):
        ...
        # The intro card is swept too: hiding is all a handle can do, so the
        # ghost keeps owning the title-keyed slot and a later same-title
        # window can end up with input but no pixels (seen 2026-08-31 after
        # repeated hot reloads). That is also why the card is no longer
        # modal - see onboarding/window.py::_build.
        # The Get in Touch form is title-keyed too - a leaked one would fight
        # the new instance over the same workspace geometry.
        try:
            import omni.ui as ui
            for handle in ui.Workspace.get_windows():
                if handle.title in (_MENU_MAIN, _MENU_ONBOARDING, _MENU_GET_IN_TOUCH):
