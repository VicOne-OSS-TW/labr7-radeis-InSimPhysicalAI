...
_EXT_ROOT = Path(__file__).parent.parent  # …/vicone.labr7.radeis/
...
def _import_tray():
    ...
    mod_name = "vicone.labr7.radeis.vlm.tray_agent_launcher"
    if mod_name in sys.modules:
        del sys.modules[mod_name]
    import vicone.labr7.radeis.vlm.tray_agent_launcher as m
    return m


def _import_sidecar_manager():
    mod_name = "vicone.labr7.radeis.vlm.sidecar_manager"
    if mod_name in sys.modules:
        del sys.modules[mod_name]
    # sidecar_manager imports sidecar_client — stub it
    sc_mod = types.ModuleType("vicone.labr7.radeis.vlm.sidecar_client")
    sc_mod.SidecarClient = MagicMock()
    sc_mod._fmt_net_err = lambda e: str(e)  # noqa: E731 -- also imported by sidecar_manager
    sys.modules["vicone.labr7.radeis.vlm.sidecar_client"] = sc_mod
    import vicone.labr7.radeis.vlm.sidecar_manager as m
    return m
...
class T6_WizardPathBConnectSpawnsTray(unittest.TestCase):
    def test_wizard_path_b_connect_spawns_tray(self):
        ...
        mod_name = "vicone.labr7.radeis.ui.model_wizard"
        if mod_name in sys.modules:
            del sys.modules[mod_name]

        # Also stub radeis_ui and other local deps
        for dep in [
            "vicone.labr7.radeis.ui.radeis_ui",
            "vicone.labr7.radeis.vlm.sidecar_client",
            "vicone.labr7.radeis.constants",
        ]:
            if dep not in sys.modules:
                sys.modules[dep] = MagicMock()

        import vicone.labr7.radeis.ui.model_wizard as wiz
        ...
        with patch("vicone.labr7.radeis.ui.model_wizard.threading") as mock_threading, \
             patch("vicone.labr7.radeis.vlm.sidecar_client.SidecarClient",
...
class T7_WizardPathBLoadSpawnsTray(unittest.TestCase):
    def test_wizard_path_b_load_spawns_tray(self):
        ...
        with patch("vicone.labr7.radeis.ui.model_wizard.threading") as mock_threading, \
             patch("vicone.labr7.radeis.ui.model_wizard.time") as mock_time:
...
def _import_tray_preflight():
    mod_name = "vicone.labr7.radeis.vlm.tray_preflight"
    if mod_name in sys.modules:
        del sys.modules[mod_name]
    import vicone.labr7.radeis.vlm.tray_preflight as m
    return m
...
def _import_sidecar_setup():
    mod_name = "vicone.labr7.radeis.ui.wizard.sidecar_setup"
    if mod_name in sys.modules:
        del sys.modules[mod_name]
    for dep in [
        "vicone.labr7.radeis.constants",
        ...
        if dep not in sys.modules:
            sys.modules[dep] = MagicMock()
    import vicone.labr7.radeis.ui.wizard.sidecar_setup as m
    return m
