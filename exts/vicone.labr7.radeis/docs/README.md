...
Radeis reports a few kinds of event, not tied to any persistent identifier, so we know roughly how many people install it, on which version, and whether a test run completes. Full field-by-field disclosure: [TELEMETRY.md](TELEMETRY.md).
...
- **Form opened**: a single `feedback_opened` count when you open the **Get in Touch** form - never its contents, and never whether you sent anything. It is the one event sent with a blank session id, so it cannot be matched up with a form submission.
...
- **Never sent**: no persistent device/user id or fingerprint, no OS build, no scene, model, or file names, paths, or outputs, no personal info. The **Get in Touch** form (the `Contact` button in the panel) is the only thing that ever sends a name or email, and only when you fill it in and submit. It sends what you type - name, email, organization, role, a use case you pick yourself from a fixed list, free-text feedback, and an "OK to email me" checkbox that defaults unchecked - to a separate `feedback` tab in a LabR7-owned Google Sheet, never joined to the events above. Every field is optional and the form submits with the box unticked. With telemetry turned off, Send makes no network call at all and tells you so.
- **Withdrawing / deletion**: to withdraw contact consent or have a submitted name, email, or message deleted from the sheet, [contact us](https://vicone.com/contact-us/). Turning telemetry off stops future sends; it does not delete an earlier submission.
- **Turn it off**: set env var `RADEIS_TELEMETRY=0`, or `exts."vicone.labr7.radeis".telemetry_enabled = false` in `extension.toml`. Opt-out returns before any network call. An in-panel toggle comes later.
